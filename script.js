document.addEventListener('DOMContentLoaded', () => {
    // ✅ A variável 'room' já é criada pelo supabase-config.js
    ...
    async function handleUpsertClient(e) { 
        ...
        if (currentEditingClient) {
            await room.collection('clients').upsert({ ...clientData, id: currentEditingClient.id });
            ...
            await room.collection('clients').upsert({ ...clientData, id: String(Date.now()) });
    ...
    async function handleDeleteClient(e) {
        ...
        if (await showConfirm('Tem certeza que deseja excluir este cliente? Isso NÃO excluirá as reservas associadas.')) {
            await room.collection('clients').delete(clientId);
        }
    }
    ...
    async function deleteBooking(bookingId) {
        if (await showConfirm('Tem certeza que deseja excluir esta reserva?')) {
            await room.collection('bookings').delete(bookingId);
            closeModal(); 
    ...
    async function handleUpsertBooking(e) {
        ...
        if (currentEditingBooking) {
            await room.collection('bookings').upsert({ ...bookingData, id: currentEditingBooking.id });
            showAlert('Reserva atualizada com sucesso!');
        } else {
            await room.collection('bookings').upsert({ ...bookingData, id: String(Date.now()) }); 
    ...
    function loadAndSubscribeData() {
        room.collection('clients').subscribe(updatedClients => {
        ...
        room.collection('bookings').subscribe(updatedBookings => {
